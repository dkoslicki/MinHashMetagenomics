...
Also, we can get the similar result by computing the ratio of the number of hash functions required for the Jaccard approach and the Jaccard estimation using the containment approach:
\begin{align*}
...
&= \frac{(c+p)}{c^2}\left(\frac{|A\cap B|}{|A\cup B|}+2(1+\delta)\frac{|A\cap B|^2}{|A\cup B|^2}+(1+\delta)^2\frac{|A\cap B|^3}{|A\cup B|^3}\right)\\
\end{align*}
Note that $\frac{|A\cap B|}{|A\cup B|} <1$. Therefore we have
\begin{align*}
...
\end{align*}
Again if we assume that $\frac{c}{c+p}\geq .9$ and recall $0<\delta<1$ we have
\begin{align*}
\frac{k_J}{k_{est}}\geq .9\frac{|B|}{|A|}\frac{1}{\left(1+2(1+\delta)+(1+\delta)^2\right)}\geq .9\frac{|B|}{|A|}\frac{1}{9} = .1\frac{|B|}{|A|}
\end{align*}
However in practice we can see that $\frac{|A\cap B|^2}{|A\cup B|^2}$ and $\frac{|A\cap B|^3}{|A\cup B|^3}$ are such small that are neglectable. Therefore in practice we will have 
 \begin{equation*}
