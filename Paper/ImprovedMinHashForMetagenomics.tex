...
To demonstrate the exponential improvement of \themethod over classical min hash for increasing sample sizes, we contrast here the mean relative performance of the classical min hash estimate to the containment approach on simulated biological data. We utilized GemSIM \cite{mcelroy2012gemsim} to simulate two sets of metagenomic data. The first set had an average number of $k$-mers in the sample was only \input{SimulatedBiologicalData_small_rel_size.txt}of the size of the average number of $k$-mers in the genomes used to simulate the data. The second set had an average number of $k$-mers in the sample equal to \input{SimulatedBiologicalData_rel_size.txt}of the average size of the number of $k$-mers in the genomes used to simulate the data. As demonstrated in Section \ref{section:ChernoffBounds} we expect that once the number of $k$-mers in the sample is large in comparison to the number of $k$-mers used to simulate the data, the containment approach will give an exponentially better estimate of the Jaccard index in comparison to the classical min hash approach. Figure \ref{fig:SimulatedBiologicalData} depicts the relative error of the classic min hash approach and the containment approach on these two sets of simulated data. Observe that the containment approach has significantly less error when, as is commonly seen in practice, the number of $k$-mers in the sample is appreciable in comparison to the number of $k$-mers in a given reference organism.

For the first set of simulated data, we used GemSIM to simulate \input{SimulatedBiologicalData_small_NumReads.txt}reads from \input{SimulatedBiologicalData_small_NumGenomes.txt}randomly selected bacterial genomes for the $k$-mer size $k=\input{SimulatedBiologicalData_small_ksize.txt}$. We then repeated this \input{SimulatedBiologicalData_small_NumReplicates.txt}times. A false positive rate of \input{SimulatedBiologicalData_small_p.txt}was used for the false positive rate of the bloom filter used for the containment approach.

For the second set of simulated data, we used GemSIM to simulate \input{SimulatedBiologicalDataNumReads.txt}reads from \input{SimulatedBiologicalDataNumGenomes.txt}randomly selected bacterial genomes for the $k$-mer size $k=\input{SimulatedBiologicalDataksize.txt}$. We then repeated this \input{SimulatedBiologicalDataNumReplicates.txt}times. A false positive rate of \input{SimulatedBiologicalDatap.txt}was used for the false positive rate of the containment approach.
...
\end{center}
\caption{Comparison of relative error of \themethod to the classical min hash estimate of the Jaccard index on simulated biological data. a) On \protect \input{SimulatedBiologicalData_small_NumReplicates.txt}replicates of samples consisting of \protect \input{SimulatedBiologicalData_small_NumGenomes.txt}genomes with only  \protect \input{SimulatedBiologicalData_small_NumReads.txt}reads. b) On \protect \input{SimulatedBiologicalDataNumReplicates.txt}replicates of samples consisting of \protect \input{SimulatedBiologicalDataNumGenomes.txt}genomes with \protect \input{SimulatedBiologicalDataNumReads.txt}reads.
}
...
To evaluate if this extremely low-abundance organism is actually present in the sample, we utilized the SNAP alignment tool \cite{zaharia2011faster} to align the sample to the \textit{Pandoravirus inopinatum} genome. The script \textit{/MakeCoveragePlot.sh} provides the exact commands and parameters used to perform the alignment. We found that \input{NumReadsAligned.txt}reads aligned with a MAPQ score above 10. The coverage of the viral genome is depicted in Figure \ref{fig:ViralCoverage} using a square-root scale and a window size of \protect \input{WindowSize.txt}\unskip. Even though the coverage was quite low (average per-window coverage was \input{MeanCoverage.txt}\unskip X), the coverage was quite uniform and consistent which lends evidence to support the claim of the presence of this particular virus in the sample metagenome.

\begin{figure}[!h]%
\begin{center}
\includegraphics[width=3.25in,trim={0 0 0 0in},clip]{Figs/CoveragePlot.png}%
\end{center}
\caption{Plot of the real metagenomic sample alignment coverage to the virus \textit{Pandoravirus inopinatum} detected by the proposed containment min hash approach. A total of  \protect \input{NumReadsAligned.txt}reads aligned with a MAPQ score above 10 using the SNAP aligner \cite{zaharia2011faster}. A square root scale and a window size of \protect \input{WindowSize.txt}was used for the plot, resulting in an average per-window coverage of \protect \input{MeanCoverage.txt}\unskip X.
}
