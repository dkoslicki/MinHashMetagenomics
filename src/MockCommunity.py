...
def align_reads(index_dir, sample_file, out_file, aligned=True):  # NOTE: snap-aligner will take SAM and BAM as INPUT!!
    if aligned:
        cmd = "/home/pi/koslickd/./snap-aligner single " + index_dir + " " + sample_file + " -o " + out_file + " -F a -t 48 -d 20 -mrl 40"  # mrl is the minimum read length that aligns (indexed with default 20, mrl=40, -d=20 -> bad alignments?
    elif not aligned:
        cmd = "/home/pi/koslickd/./snap-aligner single " + index_dir + " " + sample_file + " -o " + out_file + " -F u -t 48 -d 20 -mrl 40"
    else:
